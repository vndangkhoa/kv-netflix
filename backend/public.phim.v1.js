...
{var contentElement=jQuery('#film-content-wrapper > #film-content')[0];jQuery(contentElement).css('max-height','500px');jQuery(window).load(function(){if(typeof contentElement.scrollHeight=="number"&&contentElement.scrollHeight>0)
...
{fx.scrollTo('#film-content-wrapper',300);jQuery(contentElement).height('500px');window._flagContentHeight='small';jQuery('#btn-expand-content').text('Hiển thị thêm');}});jQuery(contentElement).css({'height':'500px','max-height':'none'});}}});};